...
export class Keyboard extends BaseRoute {
  ...
  public getSingleKeyboardByName(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    this.getMongoAccess(res)
      ...
      .subscribe(keyboardCollection => {
        if (req.query.nameLayout === "pt-br") {
          keyboardCollection
            .find({
              $and: [{ nameLayout: req.query.nameLayout }, { email: "system" }]
            })
            .toArray(function(err, keyboard) {
              res.send(keyboard);
            });
        } else {
          keyboardCollection
            .find({
              $and: [
                { nameLayout: req.query.nameLayout },
  ...
  public getSingleKeyboardByEmail(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    this.getMongoAccess(res)
      ...
      .subscribe(keyboardCollection => {
        keyboardCollection
          .find({ $and: [{ email: req.query.email }] })
          .toArray(function(err, keyboard) {
            res.send(keyboard);
          });
      });
  ...
  public getMultipleKeyboard(req: Request, res: Response, next: NextFunction) {
    this.getMongoAccess(res)
      ...
      .subscribe(keyboardCollection => {
        keyboardCollection
          .find({ $and: [{ shared: "true" }] })
          .toArray(function(err, keyboard_list) {
  ...
  public getKeyboardNames(req: Request, res: Response, next: NextFunction) {
    ...
    this.getMongoAccess(res)
      ...
      .subscribe(keyboardCollection => {
        keyboardCollection
          .find({ $or: [{ email: req.query.email }, { email: "system" }] })
          .toArray(function(err, keyboard_list) {
  ...
  public deleteKeyboard(req: Request, res: Response, next: NextFunction) {
    if (req.query.email) {
      this.getMongoAccess(res)
        ...
        .subscribe(keyboardCollection => {
          keyboardCollection
            .find({ nameLayout: req.query.nameLayout, email: req.query.email })
            .toArray(function(err, keyboard_list) {
              if (keyboard_list) {
                keyboardCollection.remove(
                  { nameLayout: req.query.nameLayout, email: req.query.email },
                  true
                );
                res.send("removed");
              } else {
  ...
  public insertUpdateOnlyKeyboard(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    ...
    this.getMongoAccess(res)
      ...
      .subscribe(keyboardCollection => {
        keyboardCollection.insert(
          this.populateLayout("pt-br", req.body.email),
          (err, result) => {}
        );
      });
  }

  public insertBasicIntoDatabase(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
