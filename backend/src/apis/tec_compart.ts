...
export class TecladoCompartihado extends BaseRoute {
  public getTecladosCompartilhados(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
  ...
  public setTecladoVisualizado(
    ...
    this.getMongoAccess(res)
      .tec_compart()
      .subscribe(tecCompartCollection => {
        tecCompartCollection
          .find({ $and: [{ id: req.query.id }] })
          .toArray(function(err, lista_teclados) {
  ...
  setTecladoCompartilhado(req: Request, res: Response, next: NextFunction) {
    this.getMongoAccess(res)
      ...
      .subscribe(tecCompartCollection => {
        tecCompartCollection
          .find({ $and: [{ id: req.body["id"] }] })
          .toArray(function(err, tec_compart_list) {
