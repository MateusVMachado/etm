...
export class LayoutEditorComponent extends AppBaseComponent implements OnInit, OnDestroy {
  ...
  private loadSingleKeyboardSubscription: Subscription;
  ...
  private saveUpdateKeyboardSubscription : Subscription;
  private subscribeToLayoutEditorSubjectSubscription : Subscription;
  private updateOnlyKeyboardSubscription : Subscription;
  private saveNewKeyboardSubscription : Subscription;
  private tecladoCompartilhadoSubscription : Subscription;
  ...
  constructor(private router: Router, 
    ...
    ngAfterViewInit() {
      //MOUSE ENTRA e SAI e tooltip se ajusta na tela para esquerda/direita
      $('.tamanho-button, .tamanho-button-especial').hover( function() {
        let el = $(this)
        let tooltipRight : number = el.offset().left + el.outerWidth() + 175;
        if(tooltipRight > $(window).width()){
          $(el).addClass('tooltipRight');
        }
        else if(el.offset().left < 175 + $('.menu-sidebar').outerWidth()){
          $(el).addClass('tooltipLeft');
        }
      });
    ...
    ngOnDestroy() {
      ...
      if(this.saveNewKeyboardSubscription) this.saveNewKeyboardSubscription.unsubscribe();
      if(this.saveUpdateKeyboardSubscription) this.saveUpdateKeyboardSubscription.unsubscribe();
      if(this.subscribeToLayoutEditorSubjectSubscription) this.subscribeToLayoutEditorSubjectSubscription.unsubscribe();
      if(this.updateOnlyKeyboardSubscription) this.updateOnlyKeyboardSubscription.unsubscribe();
      if(this.tecladoCompartilhadoSubscription) this.tecladoCompartilhadoSubscription.unsubscribe();
      if(this.tecladoCompartilhadoRemoveSubscription) this.tecladoCompartilhadoRemoveSubscription.unsubscribe();
      if(this.loadSingleKeyboardSubscription) this.loadSingleKeyboardSubscription.unsubscribe();
    ...
    */
    public updateReplicant(nameString: string){
      ...
      let user = this.authService.getLocalUser();
      if(this.loadSingleKeyboardSubscription) this.loadSingleKeyboardSubscription.unsubscribe();
      this.loadSingleKeyboardSubscription = this.tecladoService.loadSingleKeyboard(nameString, user.email, user.jwt).subscribe(async (data)=>{  
        this.convertLayoutToKeyboard(replicantFromDatabase, data[0]);
        if(data[0]) this.tecladoShare = Boolean(data[0].shared);
        if(replicantFromDatabase.magnify !== undefined ||  replicantFromDatabase.magnify !== null && !isNaN(replicantFromDatabase.magnify)){
          this.growthTextFactor = Number(replicantFromDatabase.magnify);
          if(this.growthTextFactor === 0.0) this.growthTextFactor = 1.0;
        } else {
          this.growthTextFactor = 1.00;
        }
        this.imgLinesArray = [];
    ...
    private convertLayoutToKeyboard(keyboard: TecladoModel, layout: OpenFACLayout){
      ...
      for(let i = 0; i < layout.Lines.length; i++){ 
        ...
        for(let j = 0; j < layout.Lines[i].Buttons.length; j++){ 
          ...
          if(layout.Lines[i].Buttons[j].Caption.split('$')[0] == '*img') hasImg = true;
          if(layout.Lines[i].Buttons[j].Action != 'Keyboard') hasVoice = true;
        } 
      ...
      this.thisKeyboardHasImage = hasImg;
      this.thisKeyboardHasVoice = hasVoice;
      keyboard.type = layout.nameLayout;
      this.nomeTeclado = keyboard.type;
    ...
    */
    private onDrop(value) {
      ...
      for(let unit = 0; unit < this.tecladoReplicant.teclas[drainY].length; unit++){
    ...
    */
    ...
    compartilhaTeclado(){
      let user = this.authService.getLocalUser();
      this.saveKeyboardLayout(this.newKeyboard, true);
      
    }
    descompartilhaTeclado(){
      this.saveKeyboardLayout(this.newKeyboard, false);
    }
    ...
    */
    public populateLayout(replicant: TecladoModel, email: string): OpenFACLayout{
      ...
      if(!isNullOrUndefined(replicant.shared)) openFacLayout.shared = replicant.shared;
      ...
      for(let i = 0; i < qntyLines; i++){
        ...
        for( let j = 0 ; j < teclado.teclas[i].length; j++){
          ...
          if(teclado.action[i][j] == "KeyboardAndTTS" || teclado.action[i][j] == "TTS" ){
            this.thisKeyboardHasVoice = true;
          }
    ...
    */
    public saveKeyboardLayout(saveAs?: boolean, share? : boolean){
      ...
      if(saveAs || this.newKeyboard){
        ...
        if(this.layoutEditorServiceSubscribe) this.layoutEditorServiceSubscribe.unsubscribe();
        this.layoutEditorServiceSubscribe = this.layoutEditorService.subscribeToLayoutEditorSubject().subscribe((nameArrived)=>{
          if(nameArrived && nameArrived !== 'confirm' && nameArrived !== 'refuse'){
            ...
            if(!isUndefined(share)) layout.shared = share; 
            if(this.saveNewKeyboardSubscription) this.saveNewKeyboardSubscription.unsubscribe();
            this.saveNewKeyboardSubscription = this.layoutEditorService.saveNewKeyboard(layout, user.email).subscribe((result)=>{
              let message;
              if(result === 'saved'){
                if(!isUndefined(share) && share){
                  let tecCompartilhado : TecladoCompartilhadoModel = new TecladoCompartilhadoModel();
                  tecCompartilhado.nameLayout = nameArrived;
                  tecCompartilhado.usuarioEmail = user.email;
                  tecCompartilhado.numUsado = 0;
                  tecCompartilhado.numVisualizacao = 0;
                  tecCompartilhado.keyboardImage = this.thisKeyboardHasImage;
                  tecCompartilhado.keyboardVoice = this.thisKeyboardHasVoice;
                  tecCompartilhado.id = nameArrived + user.email;
                  if(this.tecladoCompartilhadoSubscription) this.tecladoCompartilhadoSubscription.unsubscribe();
                  this.tecladoCompartilhadoSubscription = this.tecladoCompartilhadoService.compartilharTeclado(tecCompartilhado).subscribe();
                }
                else if(!isUndefined(share) && !share){
                  if(this.tecladoCompartilhadoRemoveSubscription) this.tecladoCompartilhadoRemoveSubscription.unsubscribe();
                  this.tecladoCompartilhadoRemoveSubscription = this.tecladoCompartilhadoService.removeTeclado(user.email,nameArrived).subscribe();
                }
                message = this.messageService.getTranslation('MENSAGEM_TECLADO_SALVO');
                this.messageService.success(message);
                if(share) this.tecladoShare = share
                else this.tecladoShare = false;
                this.sidebarService.emitSideBarCommand('reload');
              } else if (result === 'alreadyExist'){
                this.showModal(SaveModalComponent);
                if(this.layoutEditorServiceSubscribe2) this.layoutEditorServiceSubscribe2.unsubscribe();
                this.layoutEditorServiceSubscribe2 = this.layoutEditorService.subscribeToLayoutEditorSubject().subscribe((result)=>{
                  if(result === "confirm"){
                    ...
                    if(this.updateOnlyKeyboardSubscription) this.updateOnlyKeyboardSubscription.unsubscribe();
                    this.updateOnlyKeyboardSubscription = this.layoutEditorService.updateOnlyKeyboard(layout, user.email).subscribe((result)=>{
                      if(result === 'updated'){
                        if(!isUndefined(share) && share){
                          let tecCompartilhado : TecladoCompartilhadoModel = new TecladoCompartilhadoModel();
                          tecCompartilhado.nameLayout = nameArrived;
                ...
                this.messageService.error(this.messageService.getTranslation('MENSAGEM_MAX_TECLADOS'));
              }  
            });
        ...
        this.showModal(SaveModalComponent);
        if(this.subscribeToLayoutEditorSubjectSubscription) this.subscribeToLayoutEditorSubjectSubscription.unsubscribe();
        this.subscribeToLayoutEditorSubjectSubscription = this.layoutEditorService.subscribeToLayoutEditorSubject().subscribe((result)=>{
          if(result === "confirm"){
            let user = this.authService.getLocalUser();
            finalKeyboard.type = this.keyboardToEdit;
            finalKeyboard.magnify = this.growthTextFactor;
            ...
            if(!isUndefined(share)){
              ...
              layout.shared = this.tecladoShare;
            }
            if(this.saveUpdateKeyboardSubscription) this.saveUpdateKeyboardSubscription.unsubscribe();
            this.saveUpdateKeyboardSubscription = this.layoutEditorService.saveUpdateKeyboard(layout, user.email).subscribe((result)=>{
              let message;
              if(result === 'updated'){
                if(!isUndefined(share) && share){
                  let tecCompartilhado : TecladoCompartilhadoModel = new TecladoCompartilhadoModel();
                  tecCompartilhado.nameLayout = layout.nameLayout;
                  tecCompartilhado.usuarioEmail = user.email;
                  tecCompartilhado.numUsado = 0;
                  tecCompartilhado.numVisualizacao = 0;
                  tecCompartilhado.keyboardImage = this.thisKeyboardHasImage;
                  tecCompartilhado.keyboardVoice = this.thisKeyboardHasVoice;
                  tecCompartilhado.id = layout.nameLayout + user.email;
                  if(this.tecladoCompartilhadoSubscription) this.tecladoCompartilhadoSubscription.unsubscribe();
                  if(this.tecladoCompartilhadoRemoveSubscription) this.tecladoCompartilhadoRemoveSubscription.unsubscribe();
                  this.tecladoShare = share
                  this.tecladoCompartilhadoSubscription = this.tecladoCompartilhadoService.compartilharTeclado(tecCompartilhado).subscribe();
                }
                else if(!isUndefined(share) && !share){
                  this.tecladoShare = share
                  if(this.tecladoCompartilhadoRemoveSubscription) this.tecladoCompartilhadoRemoveSubscription.unsubscribe();
                  if(this.tecladoCompartilhadoSubscription) this.tecladoCompartilhadoSubscription.unsubscribe();
                  this.tecladoCompartilhadoRemoveSubscription = this.tecladoCompartilhadoService.removeTeclado(user.email,layout.nameLayout).subscribe();
                }
                this.messageService.success(this.messageService.getTranslation('MENSAGEM_LAYOUT_SALVO'));
                message = this.messageService.getTranslation('MENSAGEM_TECLADO_SALVO');
                this.messageService.success(message);
                this.keyboardItems.KeyboardsNames.push(this.keyboardName);
                this.sidebarService.emitSideBarCommand('reload');
    ...
    */
    public removeLine(){
      if(this.teclado.teclas.length > 5){
        ...
        this.messageService.error(this.messageService.getTranslation('MENSAGEM_MIN_LINHAS'));
      }  
    }  
    ...
    */
    public editCaptionNText(event){
      ...
      this.payloadSubscription = this.layoutEditorService.subscribeToLayoutEditorPayloadSubject().subscribe((result)=>{
        ...
        this.thisKeyboardHasImage = true;
        if(DEBUG2) console.log(JSON.stringify(this.imgLinesArray))
    ...
    */
    private mapToNewRecursive(x: number, y: number, originalArray: Array<any>, receptacleArray: Array<any>, joinOriginalArray: Array<any>,
      joinReceptacleArray: Array<any>, left: boolean, right: boolean, DEBUG: boolean){
        ...
        //check if original has even length
        ...
        //check if receptacle has even length
