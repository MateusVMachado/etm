...
export class Configuration extends BaseRoute {
    ...
    public userConfigure(req: Request, res: Response, next: NextFunction){
        if ( req.query.onlyKeyboard ){
            this.getMongoAccess(res).configurations().subscribe( (configCollection) => {
                configCollection.update({ "user": req.query.email }, { $set: {"lastKeyboard": req.query.onlyKeyboard }});
                res.status(200).send();
            });
        } else {
        ...
        this.getMongoAccess(res).configurations().subscribe( (configCollection) => {
            ...
            configCollection.find({ "user": config.user }).toArray(function(err, config_list) {
                if(config_list.length === 0){
                    configCollection.insert(config, (err, result) => {
                        res.status(200).send();
                });
            } else {
                configCollection.update({ "user": config.user }, { "openFacConfig": config.openFacConfig, "language": config.language, "user": config.user,
    ...
    public userConfigureUpdate(req: Request, res: Response, next: NextFunction){
        ...
        this.getMongoAccess(res).configurations().subscribe( (configCollection) => {
            ...
            configCollection.find({ "user": parts[2] }).toArray(function(err, config_list) {
                configCollection.update({ "user": parts[2] }, { $set: {"flexSup": parts[0] , "flexUnd": parts[1]} });
                res.status(200).send();
    ...
    public getUserConfigure(req: Request, res: Response, next: NextFunction){
        ...
        this.getMongoAccess(res).configurations().subscribe( (configCollection) => {
            ...
            configCollection.find({ "user": req.query.email }).toArray(function(err, config_list) {                    
