        ...
        logsCollection
          ...
          .toArray(function(err, userSessionList) {
            if (userSessionList.length !== 0) {
              ...
              let end = moment(
                [logoutParts[0], logoutParts[1], logoutParts[2]],
                "HH:mm:ss"
              );
              ...
              let start = moment(
                [accessParts[0], accessParts[1], accessParts[2]],
                "HH:mm:ss"
              );
              ...
              logsCollection.update(
                ...
                },
                {
                  $set: {
                    ...
                    sessDuration: userSession.sessDuration
                  }
                },
                (err, result) => {
                  res.send();
    ...
    this.getMongoAccess(res)
      ...
      .subscribe(logsCollection => {
        logsCollection
          ...
          .toArray(function(err, userSessionList) {
            logger.sumTimeIntervals(
              userSessionList,
              userSession,
              "layoutEditorIntervals",
              "layoutEditorUsage"
            );
            logsCollection.update(
              ...
              {
                $push: {
                  layoutEditorIntervals: userSession.layoutEditorIntervals
                }
              },
              (err, result) => {
                if (result) {
                  logsCollection.update(
                    ...
                    {
                      $set: { layoutEditorUsage: userSession.layoutEditorUsage }
                    },
                    (err, result) => {
                      if (result) {
                        res.send();
      ...
      .subscribe(logsCollection => {
        logsCollection
          ...
          .toArray(function(err, userSessionList) {
            ...
            logsCollection.update(
              { $and: [{ user: req.body["user"] }, { logout: "notSet" }] },
              { $push: { keyboardIntervals: userSession.keyboardIntervals } },
              (err, result) => {
                if (result) {
                  logsCollection.update(
                    {
      ...
      .subscribe(logsCollection => {
        logsCollection
          ...
          .toArray(function(err, userSessionList) {
            ...
            logsCollection.update(
              { $and: [{ user: req.body["user"] }, { logout: "notSet" }] },
              { $push: { configIntervals: userSession.configIntervals } },
              (err, result) => {
                if (result) {
                  logsCollection.update(
                    {
                    ...
                    { $set: { configUsage: userSession.configUsage } },
                    (err, result) => {
                      if (result) {
                        res.send();
                      }
