...
export class TecladoCompartihadoNota extends BaseRoute{
    ...
    public getNotaMedia(req: Request, res: Response, next: NextFunction){
        res.locals.mongoAccess.coll[5].find({"teclado": req.query.teclado}).toArray(function(err, lista_notas) {
    ...
    public getJaAvaliou(req: Request, res: Response, next: NextFunction){
        res.locals.mongoAccess.coll[5].find({"teclado": req.query.teclado,"usuarioEmail": req.query.usuarioEmail}).toArray(function(err, resposta) {
            if(resposta.length >= 1){
                res.send({value: true})
            }
            else{
                res.locals.mongoAccess.coll[5].insert(
                    {"teclado": req.query.teclado,"usuarioEmail": req.query.usuarioEmail,"nota": req.query.nota}
                );
